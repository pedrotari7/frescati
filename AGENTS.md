...
- The app is permanently dark. Colours come from the StyleX tokens in `frontend/app/tokens.stylex.ts`, `colors` for the palette and `tint` for it at less than full strength. Never hardcode hex values in components. `docs/stylex.md` covers the rest of the styling setup.
...
- **A new screen that sits below a tab needs a `backHref`, on every one of its loading and error branches too.** A tab root, the season home, Club, `/me` in a season, has none, because the tabs are how you leave it. Everything else is somebody's child.
...
- **No suite above checks a type, so `pnpm typecheck` is its own step.** Vitest and `ts-node` both transpile, eslint here has no `parserOptions.project`, and `next build` checks the app while leaving the test files out, so a green `pnpm test:all` says nothing about whether the repo compiles. Three configs, because each covers what the other two exclude: `pnpm typecheck:root` reads `shared/`'s test files, `e2e/` and `rules/`, which are in no package and were checked nowhere at all until they had a config of their own; the frontend's reads all of `frontend/`, test files included; the backend's adds `backend/scripts`. The pre-commit hook runs whichever the staged files touch. `docs/testing.md` has the table.
...
- `docs/fallow.md` — the static analysis that gates commits, over the whole repo rather than the diff: what fails and what only tells you, what it took to get the gate green and what holds it there, why CRAP is off even after the move to Istanbul coverage, the three dependencies fallow is wrong about, and why it is `pnpm fallow` rather than `pnpm exec fallow`.
...
- Prettier: tabs, width 4, printWidth 120, single quotes (including JSX), `arrowParens: avoid`. YAML is the one exception, two spaces, because YAML cannot use tabs. `pnpm format` writes; the checking form is split per component like lint and typecheck, `pnpm format:shared` and `format:check` in each package, with `pnpm format:check` covering the whole repo including everything in no package. No eslint rule reports formatting, so this is its own step for the same reason `pnpm typecheck` is; `docs/testing.md` has the why and the two gotchas.
- Pure domain logic (dates, game state, sorting) belongs in `shared/` with unit tests, not in components.

## The fallow gate

fallow is the static analysis that gates commits here. `docs/fallow.md` is the long version: what is configured and why, what to do when the gate blocks you, and what has been deliberately turned off.
...
**The gate passes, with nothing suppressed.** Dead code is at zero and holding, duplication is at zero, no function is over either complexity ceiling, and `pnpm fallow suppressions` finds no markers in the repo at all.

That is a state to hold rather than a box ticked. The scope is the whole repo, so the next dead export or clone group blocks the commit that introduces it, and it can just as easily block a commit of yours over something you did not write. When it blocks, the finding is the answer: delete the dead thing, or, if it is live and fallow cannot see the edge, say so in the config or in a `// fallow-ignore-next-line <rule>` comment with a reason. Reach for the suppression second, and never for a finding you have not checked. The five that used to be there each named a piece of work, and all five went by doing it rather than by moving a threshold.
