...
- `frontend/stylex.config.js` is the compiler's options, and it is one file because three separate compiles read it: the Rust plugin SWC runs over the app, the PostCSS pass that collects the rules, and the Babel plugin the unit tests compile with. A class name is a hash of the declaration that produced it, so two of them reading different options emit two different names for one style, and the name in the stylesheet is then not the name on the element.
- `frontend/next.config.js` compiles the styles, `frontend/postcss.config.js` collects them into the one stylesheet, `frontend/test/stylexPlugin.ts` is the unit tests' copy of the compiler, and `.browserslistrc` at the repo root says what to compile and prefix for.
...
So the check to repeat when either side moves is not a diff of the stylesheet. It is: build, collect every `x…` class the output wears, collect every one the stylesheet defines, and assert the first is a subset of the second. On a good build there are no orphans at all. `pnpm test:e2e` cannot answer this and neither can the unit suite, which compiles with Babel on both sides of the comparison and so agrees with itself.
...
## Why the tests still run Babel

The Rust compiler runs inside SWC, and the unit suite does not run SWC. Vitest compiles with Vite, so the styles have to be compiled by something Vite will run, and the Babel plugin is the compiler StyleX ships that does that. `frontend/test/stylexPlugin.ts` runs it and nothing else, over any file that mentions `@stylexjs/stylex`.

The two agree on what they emit, which is the property being relied on: the stylesheet the Rust compiler produces is byte for byte the one Babel produces, hashed class names included, because `frontend/stylex.config.js` is the one set of options both read. That is what makes a class name asserted in a test the class name that ships.

`docs/testing.md` has the rest, including why the plugin parses TypeScript and JSX rather than compiling them, and why hanging this off `@vitejs/plugin-react` stopped working.
...
- Vite hands JSX to oxc, which cannot run StyleX, so `frontend/vitest.config.ts` puts `test/stylexPlugin.ts` in front of it. Without that every suite that renders anything dies on `Unexpected 'stylex.defineVars' call at runtime`. The build no longer uses Babel; see "Why the tests still run Babel" for why this one does.
- The coverage provider had to move from istanbul to V8. `babel-plugin-istanbul` does not compose with StyleX: an instrumented `stylex.keyframes` comes out of Babel uncompiled and throws on import, and an instrumented arrow inside `stylex.create` fails the build outright. V8 counts what the engine ran and maps it back through the source maps, so nothing is rewritten to be measured. Still true, and still Babel's problem, since Babel is what compiles a test now. The thresholds have moved twice: up when the provider changed, then down again when the runner did, and `docs/testing.md` explains why the lower numbers are the honest ones.
- The hoisting rule is stricter than SWC's: a mock factory is lifted above the imports and may not reach for a variable outside its scope while it runs. Two mocks were rewritten. Both touch their outer variable lazily, from inside the function they return, which is why they survived the move to vitest unchanged.
...
Two things have since been tried against a build. `next build --turbopack` works once `next.config.js` requires `@stylexswc/nextjs-plugin/turbopack` rather than the package root, which only touches webpack, and it halves the build. It also triples the shared First Load JS and moves the stylesheet somewhere `pnpm check:stylex` cannot find it. And the whole framework was replaced with Vite, which is where `@stylexswc/rs-compiler` turns out to be callable directly, so a bundler that runs a `transform` hook can have the Rust compiler without SWC. `docs/build.md` has both, and neither is the recommendation.
