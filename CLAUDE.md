...
- **The spec files run in parallel and the two viewports do not.** `workers: 5` with `fullyParallel: false` gives each file a worker and keeps the tests inside it in order, which is what they expect, they hand state to each other on purpose. The files themselves are disjoint (responses on the next game, the kit register, the scoreline and vote on a played one, and two that write nothing at all: the admin calendar and the way back out of a screen), so they can overlap. The viewports cannot: they run the *same* specs against the *same* seeded database, so `desktop` declares `dependencies: ['mobile']`. Two consequences: asking for `--project=desktop` runs mobile first, and a red mobile leaves desktop reported as never run.
...
- `docs/finances.md` — the season's books (`seasons/{id}/dues`, `seasons/{id}/expenses`): the fees on the season, why a charge is a stored document, the sweep that raises the missing ones, paying by Swish, the lock an unpaid charge puts on signing up for another game, and chasing the people it caught.
