...
- **`E2E_STACK=vite pnpm test:e2e` runs the same specs against the Vite port** in `frontend/vite/` instead of the Next build. The specs know nothing about either, which is what makes them the evidence that the port behaves like the app. `docs/build.md` is the evaluation; the port is not what ships.
...
- `docs/build.md` — the two builds: what Next is doing that Vite would have to replace, the port in `frontend/vite/` that was written to find out, how `pnpm bench:stacks` compares them honestly, what `E2E_STACK=vite pnpm test:e2e` proved, and why the port is not the recommendation.
