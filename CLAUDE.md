...
- **The spec files run in parallel and the two viewports do not.** `workers: 7` with `fullyParallel: false` gives each file a worker and keeps the tests inside it in order, which is what they expect, they hand state to each other on purpose. The files themselves are disjoint (responses on the next game, the kit register, the scoreline and vote on a played one, the dues of two seasons, one season's receipts, and three that write nothing at all: the admin calendar, the way back out of a screen, and the headers the app serves), so they can overlap. The viewports cannot: they run the _same_ specs against the _same_ seeded database, so `desktop` declares `dependencies: ['mobile']`. Two consequences: asking for `--project=desktop` runs mobile first, and a red mobile leaves desktop reported as never run.
