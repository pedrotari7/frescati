...
/**
 * The one read in `hooks/` that isn't a listener, and the three ways it goes
 * wrong.
 *
 ...
 * on it would be thirty queries every time somebody tapped In. The other two are
 * the ordinary hazards of a promise on a screen: a read that lands after the
 * screen moved on, and one that fails and has to say so rather than draw an
 * empty season.
 */
...
describe('useSeasonResponses', () => {
	it('reads every game that still counts, once', async () => {
		...
		expect(fetched).toHaveBeenCalledWith(SEASON, ['g1', 'g2']);
	...
	it('leaves out the games nobody was asked about', async () => {
		...
		await waitFor(() => expect(fetched).toHaveBeenCalledWith(SEASON, ['g1']));
	});
	...
	it('reads again when a game is added', async () => {
		...
		await waitFor(() => expect(fetched).toHaveBeenLastCalledWith(SEASON, ['g1', 'g2']));
	});
