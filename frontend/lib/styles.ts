...
/**
 * The chrome every sheet and dialog in the app is built out of.
 *
 * Seven components declared these separately and identically: `ConfirmDialog`,
 * `DevUserSwitcher`, and the five sheets. `dialog`, `scrim`, `positioner` and
 * `title` were byte-for-byte the same in all seven, which is the kind of
 * agreement that holds until somebody changes one of them.
 *
 * `panel` and `column` are the two shapes a panel actually takes. `panel` is
 * the plain one, sized to its content. `column` is for a sheet whose middle
 * scrolls, a squad of twenty-six on a small phone, and it is a separate style
 * rather than a flag because the two differ by five properties. A panel that
 * wants a wider `maxWidth` passes its own after this one.
 *
 * `blurb` is deliberately not here. It is the one piece that genuinely differs
 * between them, 4 or 8 above, 12 or 14 across, and collapsing it would mean
 * three variants of a two-line style.
 */
