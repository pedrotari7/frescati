...
1. Enable **Firestore**, **Authentication → Google**, **Cloud Messaging**, and **Cloud Storage** (the season receipts live in the default bucket; see `docs/finances.md`).
...
4. **Authentication → Settings → Authorized domains** → add `localhost` and the domain you serve the app from. **List the apex, not each host.** The check matches a listed domain and every subdomain of it, at dot boundaries, so `frescati.xyz` covers `www.frescati.xyz` and every preview under it. See "Preview deployments" under Deployment, which is the reason previews live there at all.
...
- **Preview deployments live on `*.frescati.xyz`, not on `*.vercel.app`.** `.github/workflows/preview-domain.yml` adds a branch-linked subdomain when a pull request opens and removes it when it closes, so a branch is reachable at `<branch>.frescati.xyz` for as long as its pull request is. It needs the `PREVIEW_APEX_DOMAIN` repo variable and the same `VERCEL_*` secrets the deploy uses. Unset, the job skips.

    That is not cosmetic. Firebase Auth's authorized domains and the reCAPTCHA key behind App Check both match a listed domain plus its subdomains, at dot boundaries only. So the one `frescati.xyz` entry in each covers every branch there will ever be, and neither list is opened again. A generated Vercel hostname cannot be covered that way. `frescati-git-<branch>-<team>.vercel.app` is a single label with all three names hyphenated together, so the only pattern that matches it is `vercel.app` itself, and allowing that would let anything deployed to Vercel sign in against this project and mint its App Check tokens.

    Three things it does not fix. **Receipts do not download on a preview.** `storage.cors.json` is an exact-origin list and Cloud Storage has no subdomain matching, so a branch needs its own origin added there and deployed before `getBlob` works. Nothing else on the preview depends on it. **A preview reads and writes the live project.** A branch is editing the real group's data, and each preview origin mints its own FCM token into the real `users/{uid}/pushTokens`, after which `sw.js` can open a notification on a preview build. **Vercel Authentication still covers previews.** `ssoProtection` is `all_except_custom_domains`, which exempts the production domain only, so a preview asks for a Vercel login on either hostname. On Hobby the only option is `vercel project protection disable --sso`, which turns it off for the whole project.
